...
export class ChartState {
    ...
    _xScaleRange = $derived.by(() => {
        ...
        if (ts?.mode === 'domain' &&
            (ts.axis === 'x' || ts.axis === 'both') &&
            isScaleBand(this._xScaleProp) &&
            this.width > 0) {
            const { scale, translate } = ts;
    ...
    _rawXDomain = $derived.by(() => {
        ...
        if (ts?.mode === 'domain' &&
            (ts.axis === 'x' || ts.axis === 'both') &&
            this.width > 0) {
            return this._computeTransformDomain(this._baseXDomain, ts.translate.x, ts.scale, this.width);
        }
    ...
    _rawYDomain = $derived.by(() => {
        ...
        if (ts?.mode === 'domain' &&
            (ts.axis === 'y' || ts.axis === 'both') &&
            this.height > 0) {
            return this._computeTransformDomain(this._baseYDomain, ts.translate.y, ts.scale, this.height);
        }
